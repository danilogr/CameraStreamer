#include "AzureKinectFileReader.h"
...
// name used in logs
const char* AzureKinectFileReader::AzureKinectFileReaderConstStr = "AzureKinect";


bool AzureKinectFileReader::OpenK4AMKVFile()
{
	if (mkvPlayer)
	{
		Logger::Log(AzureKinectFileReaderConstStr) << "Device is already open!" << std::endl;
		return true;
	}

	try
	{
		mkvPlayer = k4a::device::open(K4A_DEVICE_DEFAULT);
	}
	catch (const k4a::error& e)
	{
		Logger::Log(AzureKinectFileReaderConstStr) << "Could not open default device..." << "(" << e.what() << ")" << std::endl;
		return false;
	}
	...
	try
	{
		cameraSerialNumber = mkvPlayer.get_serialnum();
	}
	catch (const k4a::error& e)
	{
		Logger::Log(AzureKinectFileReaderConstStr) << "Could not open default device..." << "(" << e.what() << ")" << std::endl;
...
void AzureKinectFileReader::saveTransformationTable(int img_width, int img_height)
{
...
void AzureKinectFileReader::CameraLoop()
{
	Logger::Log(AzureKinectFileReaderConstStr) << "Started Azure Kinect MKV Player thread: " << std::this_thread::get_id << std::endl;
	...
	while (thread_running)
	{
		...
		while (thread_running && !IsAnyCameraEnabled())
		{
			...
			{
				Logger::Log(AzureKinectFileReaderConstStr) << "Trying again in 5 seconds..." << std::endl;
				std::this_thread::sleep_for(std::chrono::seconds(5));
			}

			// try opening the device
			while (!OpenK4AMKVFile() && thread_running)
			{
				// waits one second
				std::this_thread::sleep_for(std::chrono::seconds(1));
				Logger::Log(AzureKinectFileReaderConstStr) << "Trying again..." << std::endl;
			}
			...
			Logger::Log(AzureKinectFileReaderConstStr) << "Opened Azure Kinect device SSN: " << cameraSerialNumber << std::endl;

			// starting cameras based on configuration
			try
			{
				mkvPlayer.start_cameras(&kinectConfiguration);
			...
			{
				Logger::Log(AzureKinectFileReaderConstStr) << "Error opening cameras!" << "(" << error.what() << ")" << std::endl; 
			}
			...
			if (IsAnyCameraEnabled())
			{
				try
				{
					kinectCameraCalibration    = mkvPlayer.get_calibration(kinectConfiguration.depth_mode, kinectConfiguration.color_resolution);
				...
				{
					Logger::Log(AzureKinectFileReaderConstStr) << "Error obtaining camera parameter!" << "(" << error.what() << ")" << std::endl;
			...
			if (!IsAnyCameraEnabled())
			{
				...
				colorCameraEnabled = false;
				depthCameraEnabled = false;
				Logger::Log(AzureKinectFileReaderConstStr) << "Trying again in 1 second..." << std::endl;
				std::this_thread::sleep_for(std::chrono::seconds(1));
			}
			else {
		...
		if (thread_running && IsAnyCameraEnabled())
		{
			...
			// starts
			Logger::Log(AzureKinectFileReaderConstStr) << "Started capturing" << std::endl;
			...
			try
			{
				...
				while (thread_running)
				{
					...
					if (mkvPlayer.get_capture(&currentCapture, getFrameTimeout))
					{
						...
						Logger::Log(AzureKinectFileReaderConstStr) << "Timed out while getting a frame..." << std::endl;
						--triesBeforeRestart;
						++statistics.framesFailed;

						if (triesBeforeRestart == 0)
						{
							Logger::Log(AzureKinectFileReaderConstStr) << "Tried to get a frame 5 times but failed! Restarting system in 1 second..." << std::endl;
							std::this_thread::sleep_for(std::chrono::seconds(1));
							if (mkvPlayer)
							{
								mkvPlayer.stop_cameras();
			...
			{
				Logger::Log(AzureKinectFileReaderConstStr) << "Fatal error getting frames... Restarting device in 5 seconds! (" << e.what() << ")" << std::endl;
				++statistics.framesFailed;
				if (mkvPlayer)
				{
					mkvPlayer.stop_cameras();
			...
			{
				Logger::Log(AzureKinectFileReaderConstStr) << "Fatal error! Running out of memory! Restarting device in 5 seconds! (" << e.what() << ")" << std::endl;
				++statistics.framesFailed;
				if (mkvPlayer)
				{
					mkvPlayer.stop_cameras();
		...
		if (thread_running)
		{
			Logger::Log(AzureKinectFileReaderConstStr) << "Restarting device..." << std::endl;
		}
...
bool AzureKinectFileReader::LoadConfigurationSettings()
{
	...
	if (Camera::LoadConfigurationSettings())
	{
		...
		if (configuration->IsColorCameraEnabled())
		{
			...
			default:
				Logger::Log(AzureKinectFileReaderConstStr) << "Color camera Initialization Error! The requested resolution is not supported: " << requestedWidth << 'x' << requestedHeight << std::endl;
				kinectConfiguration.color_resolution = K4A_COLOR_RESOLUTION_720P;
		...
		if (configuration->IsDepthCameraEnabled())
		{
			...
			case 1024:
				...
				break;
			default:
				Logger::Log(AzureKinectFileReaderConstStr) << "Depth camera Initialization Error! The requested resolution is not supported: " << requestedWidth << 'x' << requestedHeight << std::endl;
				kinectConfiguration.depth_mode = K4A_DEPTH_MODE_NFOV_UNBINNED;
		...
		if (canRun30fps)
		{
			...
			Logger::Log(AzureKinectFileReaderConstStr) << "WARNING! The selected combination of depth and color resolution can run at a max of 15fps!" << std::endl;
		}

		// what about the device?
		if (!configuration->UseFirstCameraAvailable())
		{
			Logger::Log(AzureKinectFileReaderConstStr) << "WARNING! We currently do not support selecting a camera based on serial number (Sorry!)" << std::endl;
		}
